...
if(isset($_GET['id'])){
	...
	if(isset($_POST['submit'])){ 
		...
		OR empty($_POST['check_in']) OR empty($_POST['check_out'])){
			...
			$user_id = $_SESSION['id'];
			$status = "Pending"; //static, pending by default, shows the condition of the booking
			...
			//grabbing price through session 
			 ...
			 $in_date = strtotime($_POST['check_in']);
			 $out_date = strtotime($_POST['check_out']);
	   
			 if($curr_date > $in_date OR $curr_date > $out_date){
	   
			   echo "<script> alert('pick a date that in not in the past, pick starting from tomorrow') </script>";
	   
			 }else{
			   if($in_date > $out_date OR $in_date == $curr_date){
					echo "<script> alert('pick a date that is not today for check-in and pick a check-in date that is less than check-out date') </script>";
					...
					$booking->execute([
						...
						":check_out" => $_POST['check_out'], //the variables will insert them as dates, but i want to insert them as strings, so we use post
						":email" => $email,
					...
					//echo "<script>  window.location.href='pay.php'; </script>"; //javascript way
					echo "<script> window.location.href='".APPURL."/rooms/pay.php' </script> "; //php way, after we finsih booking we will go to pay.php
					//the path is index page then rooms then pay.php 
